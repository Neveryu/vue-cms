<template>
  <div class="container">
    ...
    <!-- 精简模式 -->
    <expand-model v-if="showModel === 'expand'" :tableDatas="fileList" :selectionArr="checkList" @delOne="delFunc"></expand-model>

    <!-- 删除素材时的弹窗 -->
    <el-dialog title="删除" class="del-dialog" :visible.sync="showDelDialog" :close-on-click-modal="false" :close-on-press-escape="false">
...
/*删除时的弹窗*/
.del-dialog ::v-deep .el-dialog {
  max-width: 500px;
  border-radius: 4px;
}
.del-dialog ::v-deep .el-dialog__header {
  padding: 10px;
...
.del-dialog ::v-deep .el-dialog__footer {
  padding: 6px 0;
...
.del-dialog ::v-deep .el-dialog__title {
  font-size: 15px;
  font-weight: bold;
}
.del-dialog ::v-deep .el-dialog__close {
  position: relative;
  top: -10px;
