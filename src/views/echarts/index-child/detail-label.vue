  ...
  <div class="detail-wrapper">
    <div class="label-scroll" ref="labelWrapper">
      <ul class="label-wrapper" ref="scrollParent">
        <li class="label" :class="{ active: index === activeItem }" v-for="(item, index) of labelData" :key="item.id" @click="selectItem(index)">
          {{ item.quesName }}
        </li>
      </ul>
    </div>
...
.detail-wrapper {
  ...
  .label-scroll {
    font-size: 0;
    width: 100%;
    background-image: url('~@/assets/image/echarts/label-bg.png');
